...
std::string Sha256::hash_value(std::string msg)
{
    h0 = 0x6a09e667;
    h1 = 0xbb67ae85;
    h2 = 0x3c6ef372;
...
std::string Sha256::char_string_to_binary_string(std::string str,unsigned int size)
{
    std::string output="";
...
unsigned int Sha256::calculate_k(unsigned int message_length_of_bits)
{
...
std::string Sha256::append_big_endian(unsigned int message_length_of_bits)
{
    std::string output="";
    output=std::bitset<64>(message_length_of_bits).to_string();
    return output;
}
unsigned int Sha256::string_of_32bits_to_int(std::string number_as_string)
{
    return stoi(number_as_string,0,2);
}
std::string Sha256::right_rotation(std::string str,unsigned int rotation_value)
{
    return str.substr(rotation_value,str.size())+str.substr(0,rotation_value);
}
std::vector<unsigned int> Sha256::extending_16_words_to_64_words(std::string message)
{
...
unsigned int Sha256::integer_right_rotation(unsigned int number, unsigned int d)
{
    return(number >> d) | (number << (unsigned)(32-d));
}
std::string Sha256::hex_to_string(unsigned int number)
{
    std::stringstream ss;
    ss << std::hex << number;
...
std::string Sha256::compression(std::vector<unsigned int> w)
{
...
std::string Sha256::ascii_string_from_binary_string(std::string str)
{
	std::string output="";
